// src/scrape.js
// Parcourt toutes les ventes -> tous les lots -> filtre "montre|montres|horlogerie"
// Génère public/montres.html avec un compte à rebours jusqu'au prochain run (10:00 Europe/Paris)
...
const RUN_HOUR = Number(process.env.RUN_HOUR || 10);
const RUN_MINUTE = Number(process.env.RUN_MINUTE || 0);

const sleep = (ms) => new Promise(r => setTimeout(r, ms));
const toAbs = (base, href) => { try { return new URL(href, base).toString(); } catch { return href; } };
const nextRun = () => {
  const now = DateTime.now().setZone(TZ);
  let next = now.set({ hour: RUN_HOUR, minute: RUN_MINUTE, second: 0, millisecond: 0 });
...
function nextFromVentes(html, pageUrl) {
  const $ = cheerio.load(html);
  const sel = $("a[rel='next'], a.next, li.pagination-next a, a[aria-label*='Suivant'], a:contains('Suivant'), a:contains('suivant')");
...
<footer>
  <p>Source : <a href="${BASE}" target="_blank" rel="noopener">${BASE}</a>. Page régénérée automatiquement chaque jour à ${String(RUN_HOUR).padStart(2,"0")}:${String(RUN_MINUTE).padStart(2,"0")} (${TZ}).</p>
</footer>
<script>
(function(){
